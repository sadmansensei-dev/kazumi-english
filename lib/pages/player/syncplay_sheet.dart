...
class _SyncPlayHomeSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(builder: (context) {
      ...
      return _SyncPlaySheetScaffold(
        ...
        description: 'Sync play, pause, and episode selection with friends',
        primaryAction: hasSession
            ? FilledButton.tonalIcon(
                ...
                label: Text(connecting ? 'Cancel Connection' : 'Disconnect'),
              )
            : null,
  ...
  Widget _buildLobby(BuildContext context, {required bool compact}) {
    ...
    final Widget create = _ChoiceCard(
      ...
      title: 'Create Room',
      description: 'Generate a room code and invite friends',
      emphasized: true,
    ...
    final Widget join = _ChoiceCard(
      ...
      title: 'Join Room',
      description: "Have a friend's room code",
      emphasized: false,
...
class _SyncPlayRoomSheetState extends State<_SyncPlayRoomSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    return _SyncPlaySheetScaffold(
      ...
      description: isCreate ? 'Share the room code with friends' : "Enter a friend's room code",
      showCancel: true,
      primaryAction: FilledButton.icon(
        onPressed: _submit,
        icon: Icon(
            isCreate ? Icons.play_circle_outline_rounded : Icons.login_rounded),
        label: Text(isCreate ? 'Create and Join' : 'Join Room'),
      ),
      bodyBuilder: (context, compact) {
    ...
    return TextFormField(
      ...
      validator: (value) {
        ...
        if (!RegExp(r'^[0-9]{6,10}$').hasMatch(text)) {
          return 'Room code is 6-10 digits';
        }
        return null;
      },
    );
  ...
  Widget _buildUsernameField({bool compact = false}) {
    return TextFormField(
      ...
      decoration: _sheetInputDecoration(
        ...
        helperText: compact ? null : '4-12 letters, visible to others in the room',
      ),
      validator: (value) {
        final String text = (value ?? '').trim();
        if (text.isEmpty) {
        ...
        if (!RegExp(r'^[a-zA-Z]{4,12}$').hasMatch(text)) {
          return 'Nickname is 4-12 letters';
        }
        return null;
      },
...
class _SyncPlayServerSheetState extends State<_SyncPlayServerSheet> {
  ...
  static const String _customOption = 'Custom Server';
  ...
  void _save() {
    ...
    if (endPoint == _customOption) {
      ...
      if (parseSyncPlayEndPoint(endPoint) == null) {
        setState(() => _customEndPointError = 'Address format is host:port');
        return;
      }
    }
  ...
  Widget build(BuildContext context) {
    return _SyncPlaySheetScaffold(
      title: 'Sync Server',
      description: 'Room members must use the same server',
      showCancel: true,
