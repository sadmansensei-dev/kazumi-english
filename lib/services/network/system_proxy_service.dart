...
class SystemProxyService {
  ...
  static bool refresh() {
    ...
    try {
      ...
      KazumiLogger().w('Proxy: failed to read system proxy configuration $e');
    ...
    KazumiLogger().i(proxy == null
        ? 'Proxy: system proxy removed, falling back to direct connection'
        : 'Proxy: system proxy detected ${proxy.$1}:${proxy.$2}');
    return true;
  }
  ...
  static void _startWatcher() {
    ...
    using((arena) {
      ...
      if (RegOpenKeyEx(HKEY_CURRENT_USER, subKey, 0, KEY_NOTIFY, phKey) !=
          ERROR_SUCCESS) {
        KazumiLogger().w('Proxy: failed to start system proxy monitor (failed to open registry key)');
        return;
      }
      _hKey = phKey.value;
      ...
      if (_hEvent == 0) {
        _stopWatcher('CreateEvent failed');
        return;
      }
      ...
      if (_registerWaitForSingleObject(phWait, _hEvent,
          ...
          0) {
        _stopWatcher('RegisterWaitForSingleObject failed');
        return;
      }
  ...
  static bool _armNotification() {
    ...
    if (status != ERROR_SUCCESS) {
      KazumiLogger().w('Proxy: failed to register system proxy change notification ($status)');
      return false;
    }
    return true;
  }
  ...
  static void _stopWatcher(String? reason) {
    if (reason != null) {
      KazumiLogger().w('Proxy: failed to start system proxy monitor ($reason)');
    }
    _waitCallback?.close();
    _waitCallback = null;
    if (_hEvent != 0) {
