...
class PluginCatalogView extends StatefulWidget {
  const PluginCatalogView({
    ...
    this.errorMessage = 'Unable to access the rule repository',
  });
...
class PluginCatalogViewState extends State<PluginCatalogView> {
        ...
        itemBuilder: (context, index) {
          ...
          return RuleCard(
            ...
            trailing: RuleCardActionButton(
              label: switch (status) {
                PluginCatalogItemStatus.install => 'Install',
                PluginCatalogItemStatus.installed => 'Installed',
                PluginCatalogItemStatus.update => 'Update',
              },
  ...
  Widget _buildLoadError() {
    ...
    return Center(
      child: GeneralErrorWidget(
        errMsg:
            '${widget.errorMessage}\n${enableGitProxy ? 'Rule repo mirror enabled' : 'Rule repo mirror disabled'}',
        actions: [
          GeneralErrorButton(
            onPressed: () => unawaited(_toggleGitProxyAndRefresh()),
            text: enableGitProxy ? 'Disable Rule Mirror' : 'Enable Rule Mirror',
          ),
          GeneralErrorButton(onPressed: refresh, text: 'Refresh'),
        ],
      ),
    );
  }
  ...
  Widget _buildBody() {
    ...
    if (_controller.pluginHTTPList.isEmpty) {
      return const Center(child: Text('No rules in the rule repository yet'));
    }
