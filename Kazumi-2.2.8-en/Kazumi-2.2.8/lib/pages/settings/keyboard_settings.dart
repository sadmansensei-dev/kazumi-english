...
const List<_ShortcutGroup> _shortcutGroups = [
  _ShortcutGroup('Playback Controls', Icons.play_arrow_rounded,
      ['playorpause', 'forward', 'rewind', 'skip', 'next', 'prev']),
  _ShortcutGroup(
      'Volume', Icons.volume_up_rounded, ['volumeup', 'volumedown', 'togglemute']),
  _ShortcutGroup('Video & Danmaku', Icons.fullscreen_rounded,
...
class _KeyboardSettingsPageState extends State<KeyboardSettingsPage> {
    ...
    for (final entry in shortcuts.entries) {
      ...
      for (int i = 0; i < otherKeys.length; i++) {
        ...
        if (otherKeys[i] == rawKey) {
          ...
          KazumiDialog.showToast(message: "This key is already used by [$name], please choose another");
          return true;
        }
      }
    }
  ...
  void restoreDefaults() {
    ...
    KazumiDialog.showToast(message: 'Shortcuts restored to default');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SettingsDetailScaffold(
      title: const Text('Action Settings'),
      actions: [
        IconButton(
              ...
              Center(
                child: ConstrainedBox(
                  ...
                  child: Padding(
                    ...
                    child: Text(
                      'Tap the key label, then press a new key to change it',
  ...
  Widget _buildKeyCap(String func, List<String> keys, int i) {
    ...
    return _KeyCap(
      label: listening ? 'Press Any Key' : keyAliases[keys[i]] ?? keys[i],
      listening: listening,
