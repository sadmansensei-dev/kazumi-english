...
class _DanmakuShieldSettingsState extends State<DanmakuShieldSettings> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: SysAppBar(
        title: const Text("Danmaku Filter"),
      ),
      body: ListView(
        ...
        children: [
          ...
          SizedBox(height: 12),
          Text(
            'Starting and ending with "/" will be treated as a regular expression, e.g. "/\\d+/" blocks all digits',
          ),
          Observer(builder: (context) {
            return Text(
              "Added ${myController.shieldList.length} keyword(s)",
            );
          }),
          SizedBox(height: 12),
          Observer(builder: (context) {
