...
abstract class _CollectController with Store {
  ...
  Future<_BangumiDeleteSyncAction?> _resolveBangumiDeleteSyncAction(
    ...
    return KazumiDialog.show<_BangumiDeleteSyncAction>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'For security reasons, Bangumi does not provide a delete API. You can mark both local and remote as "Dropped", or delete only the local collection and manually remove the Bangumi data via the website.',
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> _openBangumiSubjectPage(int bangumiId) async {
    ...
    KazumiDialog.showToast(message: 'Unable to open the Bangumi webpage');
  }
  ...
  Future<bool> _syncBangumiCollectIfEnabled(
    ...
    final bangumi = BangumiSyncService();
    if (!bangumi.initialized) {
      KazumiDialog.showToast(message: 'Bangumi is not initialized; sync failed, this status change was cancelled');
      KazumiLogger().w(
    ...
    try {
      ...
      if (synced && showImmediateSyncToast) {
        ...
        KazumiDialog.showToast(message: 'Failed to sync to Bangumi, this status change was cancelled');
        KazumiLogger().w(
      ...
      KazumiDialog.showToast(message: 'Failed to sync to Bangumi, this status change was cancelled: $e');
      KazumiLogger().e(
  ...
  Future<bool> syncCollectiblesBangumi(
    ...
    if (!BangumiSyncService().initialized) {
      KazumiDialog.showToast(message: 'Bangumi sync is enabled but not initialized, please check your Token and retry');
      return false;
    }
    try {
      ...
      try {
        ...
        if (showSuccessToast) {
          KazumiDialog.showToast(
            message: hasChanges ? 'Bangumi sync complete' : 'No status difference found, no sync needed',
          );
        }
      } catch (e) {
