...
abstract class _PlayerSyncPlayController with Store {
  ...
  Future<void> createRoom(
    ...
    try {
      ...
      client.onGeneralMessage.listen(
        ...
        onError: (error) {
          ...
          if (error is SyncplayConnectionException) {
            exitRoom();
            KazumiDialog.showToast(
              message: 'SyncPlay: sync interrupted $message',
      ...
      client.onRoomMessage.listen(
        (message) {
          ...
          if (message['type'] == 'init') {
            if (message['username'] == '') {
              KazumiDialog.showToast(
                  message: 'SyncPlay: you are the only user in this room',
                  duration: const Duration(seconds: 5));
              setPlayingBangumi();
            } else {
              KazumiDialog.showToast(
                  message:
                      'SyncPlay: you are not the only user in this room; progress now follows user ${message['username']}');
            }
          }
      ...
      KazumiDialog.showToast(
        message: 'SyncPlay: connection failed $message',
        duration: const Duration(seconds: 5),
      );
    }
  }
