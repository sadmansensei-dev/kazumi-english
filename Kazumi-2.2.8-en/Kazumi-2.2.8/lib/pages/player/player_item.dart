...
class _PlayerItemState extends State<PlayerItem>
  ...
  Future<void> handlePreNextEpisode(String direction) async {
    ...
    if (targetEpisode > episodes.length) {
      KazumiDialog.showToast(message: 'This is already the latest episode');
      return;
    }
    if (targetEpisode <= 0) {
      KazumiDialog.showToast(message: 'This is already the first episode');
      return;
    }
    ...
    if (targetRef != null) {
      KazumiDialog.showToast(message: 'Loading ${targetRef.displayTitle}');
    }
  ...
  Future<void> handleScreenshot() async {
    ...
    if (isDesktop()) {
      KazumiDialog.showToast(message: 'Saving screenshots is not yet supported on desktop');
      return;
    }
    ...
    try {
      ...
      if (screenshot == null) {
        KazumiDialog.showToast(message: 'Screenshot failed: no image captured');
        return;
      }
      ...
      if (!result.isSuccess) {
        KazumiDialog.showToast(message: 'Failed to save screenshot: ${result.errorMessage}');
      }
    } catch (e) {
    ...
    if (Platform.isAndroid && mode != SuperResolutionMode.off) {
      ...
      if (androidVideoRenderer == 'mediacodec_embed') {
        await KazumiDialog.show(builder: (context) {
          return AlertDialog(
            title: const Text('Compatibility Notice'),
            content: const Text('The MediaCodec renderer does not support super resolution.\n\n'
        ...
        return StatefulBuilder(builder: (context, setState) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                const Text('Enabling super resolution (quality tier) may cause your device to lag. Continue?'),
    ...
    try {
      ...
      if (type == "up") {
        if (index < defaultPlaySpeedList.length - 1) {
          ...
          KazumiDialog.showToast(message: 'Maximum speed reached');
        }
      } else if (type == "down") {
        if (index > 0) {
          index--;
          ...
          KazumiDialog.showToast(message: 'Minimum speed reached');
        }
      }
    } catch (e) {
    ...
    return Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (playerController.playback.completed && !videoPageController.loading) {
        if (playerController.playback.resumedNearEnd) {
          ...
          if (nextRef != null) {
            KazumiDialog.showToast(message: 'Loading ${nextRef.displayTitle}');
          }
          try {
            playerTimer!.cancel();
          } catch (_) {}
                              ...
                              onTap: () async {
                                ...
                                try {
                                  ...
                                  if (hasDanmakus) {
                                    ...
                                    KazumiDialog.showToast(message: 'Danmaku toggled successfully');
                                  } else {
                                    playerController.danmaku
                                        .setDanmakuEnabled(false);
                                    KazumiDialog.showToast(message: 'No danmaku content found');
                                  }
                                } catch (e) {
                                  KazumiDialog.showToast(message: 'Failed to toggle danmaku');
                                }
                              },
                            );
                          }).toList(),
