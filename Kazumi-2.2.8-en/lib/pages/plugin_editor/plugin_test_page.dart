...
class _PluginTestPageState extends State<PluginTestPage> {
  ...
  String _getSearchSubtitle() {
    ...
    if (!_hasSearchRaw) return 'Test not run';
    return '${plugin.searchMode == RuleMode.api ? 'JSON' : 'HTML'} length: ${searchRaw.length} chars';
  }

  // 简化副标题颜色逻辑：仅三类
  Color _getSubtitleColor(String subtitle, ThemeData theme) {
    if (subtitle.contains('Testing') ||
        subtitle.contains('Fetching') ||
        subtitle.contains('Parsing')) {
      return theme.getCoreColor(CoreColorType.waiting);
    }
    if (subtitle.contains('Failed') ||
        subtitle.contains('None available') ||
        subtitle.contains('None valid') ||
        subtitle.contains('Skip')) {
  ...
  Widget _buildSearchContent(ThemeData theme) {
    ...
    if (!_hasSearchRaw) return _buildEmpty('Tap "Start Test" at the top to run', theme);
    return Container(
  ...
  String _getParseSubtitle() {
    if (isTesting && _shownFragmentIndex == null) return 'Parsing...';
    if (!_hasSearchRaw) return 'Parsing not run';
    if (!_hasSearchData) return 'No results parsed';
    final skipped =
        searchDiagnostics.isEmpty ? '' : ', ${searchDiagnostics.length} skipped';
    return 'Parsed ${searchRes?.data.length ?? 0} result(s)$skipped';
  }

  Widget _buildParseContent(ThemeData theme) {
    if (isTesting && _shownFragmentIndex == null) return _buildLoading(theme);
    if (!_hasSearchRaw) return _buildEmpty('Please complete the search request test first', theme);
    if (!_hasSearchData) return _buildEmpty('No search results parsed', theme, isError: true);
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            Text('Some nodes were skipped (${diagnostics.length})',
                style: theme.textTheme.bodyMedium?.copyWith(
                    color: theme.colorScheme.onErrorContainer,
  ...
  Widget _buildSearchItemCard(SearchItem item, int i, ThemeData theme) {
    ...
    final fragment = _itemFragmentMap[i] ?? 'No matching segment';
          ...
          child:
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              IconButton(
                ...
                tooltip: isShowingFragment ? 'Hide Matched Segment' : 'View Matched Segment',
              ),
            ]),
            _h8,
            Text('Link: ${item.src}',
  ...
  String _getChapterSubtitle() {
    if (isTesting) return 'Fetching...';
    if (!_hasSearchData) return 'No valid search results';
    if (!_needChapterParse) return 'No episode parsing needed';
    if (chapters == null) return 'Episode data not obtained';
    final skipped =
        chapterDiagnostics.isEmpty ? '' : ', ${chapterDiagnostics.length} skipped';
    return 'Got ${chapters?.length ?? 0} playback source(s)$skipped';
  }

  Widget _buildChapterContent(ThemeData theme) {
    if (!_needChapterParse) return _buildEmpty('Episode rule not filled in', theme);
    if (isTesting) return _buildLoading(theme);
    if (!_hasSearchData) return _buildEmpty('Please parse a valid result first', theme);
    if (chapters == null) return _buildEmpty('Episode data not obtained', theme, isError: true);
    if (!_hasChapters) return _buildEmpty('No episodes available', theme, isError: true);
  ...
  Widget _buildChapterCard(Road road, int i, ThemeData theme) => Card(
        ...
        child: Padding(
          ...
          child: Column(
              ...
              children: [
                Text(
                  'Source ${i + 1}: ${road.name}',
                  style: theme.textTheme.titleMedium
                      ?.copyWith(fontWeight: FontWeight.w500),
                ),
                _h8,
                Text('Episode count: ${road.data.length}',
                    style: theme.textTheme.bodySmall?.copyWith(
                        color: theme.getCoreColor(CoreColorType.waiting))),
                _h8,
                SizedBox(
