    ...
    try {
      ...
      if (proxyEnable) {
        ...
        if (proxy != null) {
          ...
          KazumiLogger().i('Player: following system proxy http://${proxy.$1}:${proxy.$2}');
        }
      }
        ...
        if (showPlayerError) {
          ...
          if (event.toString().contains('Failed to open') && playerBuffering) {
            KazumiDialog.showToast(
                message: 'Failed to load, try switching to another video source', showActionButton: true);
          } else {
            KazumiDialog.showToast(
                message: 'Internal player error ${event.toString()} ${videoUrl()}',
      ...
      if (cachePolicy.networkForced) {
        KazumiDialog.showToast(message: 'Using mobile data; low memory mode temporarily enabled to reduce caching');
      }
