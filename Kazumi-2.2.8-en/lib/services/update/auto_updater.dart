...
class AutoUpdater {
    ...
    try {
      ...
      if (needUpdate(currentVersion, remoteVersion)) {
        ...
        return UpdateInfo(
          ...
          description: data['body'] ?? 'New Version Found',
          downloadUrl: '',
          // 将在用户选择安装类型后填充
  ...
  Future<void> manualCheckForUpdates() async {
    try {
      ...
      if (updateInfo != null) {
        ...
        KazumiDialog.showToast(message: 'You already have the latest version!');
      }
    } catch (e) {
  ...
  void _showUpdateDialog(UpdateInfo updateInfo, {bool isAutoCheck = false}) {
    KazumiDialog.show(
      builder: (context) {
        return AlertDialog(
          title: Text('New version found ${updateInfo.version}'),
  ...
  Future<void> _downloadUpdate(
    ...
    try {
      ...
      if (e.toString().contains('Permission denied') ||
        ...
        errorMessage = "Insufficient permissions, the file has been saved to the app's temporary directory";
        ...
        errorMessage = 'File integrity verification failed, possibly due to a network transfer error';
      }
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text('New version ${updateInfo.version} downloaded'),
                  ),
                ],
              ),
  ...
  void _installUpdate(
    ...
    try {
      // 显示准备退出的提示
      KazumiDialog.showToast(message: 'Preparing to install update, the app will now exit...');
      ...
      if (Platform.isWindows) {
        ...
        if (result.type != ResultType.done) {
          KazumiDialog.showToast(message: 'Unable to open installation file: ${result.message}');
          return;
        }
      }
    } catch (e) {
      KazumiDialog.showToast(message: 'Failed to launch installer: ${e.toString()}');
  ...
  void _revealInFileManager(String filePath) async {
    try {
      ...
      KazumiDialog.showToast(message: 'Unable to open file manager');
