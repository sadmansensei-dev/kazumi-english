...
class HistoryPlaybackService {
  ...
  Future<HistoryPlaybackResult> open(
    ...
    if (HistoryEntryKind.normalize(history.entryKind) ==
      ...
      return args == null
          ? const HistoryPlaybackUnavailable('No usable cache found')
          : HistoryPlaybackReady(args);
    }

    final args = await _onlineArgs(history, cancelToken);
    return args == null
        ? const HistoryPlaybackUnavailable('Online source unavailable, please choose another playback source')
        : HistoryPlaybackReady(args);
  }
