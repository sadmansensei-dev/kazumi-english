...
class RestrictedJsonPath {
  ...
  static void validate(String expression) {
    if (expression.isEmpty || !expression.startsWith(r'$')) {
      throw ApiRuleFormatException('JSONPath must start with \$: $expression');
    }
    var index = 1;
    while (index < expression.length) {
      ...
      if (char == '.') {
        ...
        if (index == start) {
          throw ApiRuleFormatException('Unsupported JSONPath: $expression');
        }
        continue;
      }
      if (char == '[') {
        ...
        if (!isIndex && !isWildcard && !isQuoted) {
          throw ApiRuleFormatException('Unsupported JSONPath segment: [$content]');
        }
        index = end + 1;
        continue;
      }
  ...
  static int _findBracketEnd(String expression, int start) {
    ...
    throw ApiRuleFormatException('JSONPath missing ]: $expression');
  }
  ...
  static List<Object?> read(dynamic document, String expression) {
    ...
    try {
      ...
      throw ApiRuleFormatException('Failed to parse JSONPath $expression: $error');
    }
  }
...
class ApiRuleStrategy {
  ...
  dynamic decodeResponse(String raw) {
    try {
      ...
      throw ApiRuleFormatException('API response is not valid JSON: ${error.message}');
    }
  }
  ...
  PreparedRuleRequest prepareRequest(
    ...
    if (method != 'GET' && method != 'POST') {
      throw ApiRuleFormatException('Only GET/POST are supported, current is $method');
    }
    if (config.url.trim().isEmpty) {
      throw const ApiRuleFormatException('API request URL cannot be empty');
    }
    final url = _renderTemplate(config.url.trim(), variables, encode: true);
    final uri = Uri.tryParse(url);
    if (uri == null || !uri.hasScheme || uri.host.isEmpty) {
      throw ApiRuleFormatException('API request URL is invalid: $url');
    }
    ...
    for (final entry in config.variables.entries) {
      ...
      if (value == null) {
        throw ApiRuleFormatException(
          'Episode response variable ${entry.key} did not match a value: ${entry.value}',
        );
      }
      ...
      try {
        ...
        for (var episodeIndex = 0;
          ...
          try {
            ...
            urls.add(pageUrl);
            names.add(
              episodeName.isEmpty ? 'Episode ${episodeIndex + 1}' : episodeName,
            );
          } on ApiRuleFormatException {
            rethrow;
          } catch (error) {
        ...
        if (urls.isEmpty) {
          diagnostics.add('Source node $roadIndex has no valid episodes, skipped');
          continue;
        }
        roads.add(
          Road(
            name: roadName.isEmpty
                ? 'Source ${roads.length + 1}'
                : roadName,
            data: urls,
            identifier: names,
          ),
  ...
  List<Road> _parseDelimited(
    ...
    for (var roadIndex = 0; roadIndex < roadGroups.length; roadIndex++) {
      ...
      for (var episodeIndex = 0;
        ...
        if (separatorIndex < 0) {
          diagnostics.add(
            'Source $roadIndex episode entry $episodeIndex is missing a field separator, skipped',
          );
          continue;
        }
        ...
        try {
          ...
          if (pageUrl.isEmpty) {
            diagnostics.add(
              'Source $roadIndex episode entry $episodeIndex is missing a URL, skipped',
            );
            continue;
          }
          urls.add(pageUrl);
          ...
          diagnostics.add(
            'Failed to parse source $roadIndex episode entry $episodeIndex: $error',
          );
        }
      }
      if (urls.isEmpty) {
  ...
  String _resolveEpisodeUrl(
    ...
    if (page.url.trim().isEmpty) {
      throw const ApiRuleFormatException('Playback page URL template cannot be empty');
    }
    ...
    if (uri == null) {
      throw ApiRuleFormatException('Invalid episode page URL: $path');
    }
  ...
  void validateChapterConfig(ApiChapterConfig config) {
    ...
    if (config.format == ApiChapterFormat.delimited) {
      ...
      if (config.roadSeparator.isEmpty ||
        ...
        throw const ApiRuleFormatException('Episode separator cannot be empty');
      }
      return;
    }
    ...
    if (config.episodeUrlPath.trim().isNotEmpty) {
      ...
      throw const ApiRuleFormatException(
        'You must configure either the playback entry URL path or the playback page URL template',
      );
    }
    ...
    if (value is String) {
      ...
      if (exact != null) {
        ...
        if (!variables.containsKey(name)) {
          throw ApiRuleFormatException('Missing template variable @$name');
        }
