...
class BangumiSyncService {
  ...
  Future<void> init() async {
    ...
    if (_configuredToken.isEmpty) {
      throw Exception('Please enter your Bangumi Access Token first');
    }
    try {
      await ping();
      initialized = true;
  ...
  Future<void> ping() async {
    if (isUsing) {
      throw Exception('Bangumi: An operation is already in progress, please try again later');
    }
  ...
  Future<bool> syncCollectibles({
    ...
    if (!syncEnable) {
      KazumiDialog.showToast(message: 'Sync Disabled');
    ...
    return _runExclusive(() async {
      try {
        onProgress?.call('Start syncing Bangumi status', 0, 0);
        ...
        if (totalOperations == 0) {
          onProgress?.call('No status difference found, no sync needed', 1, 1);
          return false;
        }
        ...
        if (mergePlan.localOnlyUploads.isNotEmpty) {
          ...
          for (final upload in mergePlan.localOnlyUploads) {
            ...
            if (!updated) {
              onProgress?.call('Failed to upload new local status', syncedCount, totalOperations);
              throw Exception('Sync failed: item ${upload.bangumiId} failed to upload to Bangumi');
            }
            syncedCount++;
            onProgress?.call('Uploading newly added local status', syncedCount, totalOperations);
          }
